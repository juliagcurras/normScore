...
#' computed for each normalization method and group, and displayed as point-range
#' or boxplot-based summaries depending on the number of groups.
#'
...
plotItem2 <- function(
  ...
  dfPlot$Normalization <- factor(
    dfPlot$Normalization,
  ...
  ggplot2::ggplot(
    ...
    ggplot2::scale_colour_manual(values = normScorePalette(length(levels(dfPlot$Normalization)))) +
    ggplot2::labs(
      x = "Normalization Methods",
      y = "Intragroup correlation",
      colour = "Normalization",
      title = "Intragroup correlation - Item 2"
    ) +
    ggplot2::theme_minimal(base_size = 14) +
    ggplot2::theme(
      axis.line = ggplot2::element_line(linewidth = 0.5, colour = "black"),
...
#'
#' Generates MA plots for each normalization method using the selected reference
#' and alternative groups. These plots are used to assess intensity-dependent
...
plotItem3 <- function(
  ...
  minY <- floor(min(sapply(dataToPlot, function(df) min(df$M, na.rm = TRUE))))*1.1
  maxY <- ceiling(max(sapply(dataToPlot, function(df) max(df$M, na.rm = TRUE))))*1.1
    ...
    FUN = function(nameNorm) { 
      ...
      ggplot2::ggplot(
        ...
        ggplot2::geom_point(
          ...
          na.rm = TRUE,
          size = 1.5, 
          fill = colors[[nameNorm]], 
          colour = colors[[nameNorm]]
        ) +
        ggplot2::geom_hline(
          yintercept = 0,
          linetype = "dashed",
...
plotItem4 <- function(
  ...
  # Common parameters for plotting
  minY <- floor(min(sapply(dataToPlot, function(df) min(df$SD, na.rm = TRUE))))*1.1
  maxY <- ceiling(max(sapply(dataToPlot, function(df) max(df$SD, na.rm = TRUE))))*1.1
  colors <- normScorePalette(length(dataToPlot))
  names(colors) <- names(dataToPlot)
  
  # Plot!
  allMeaSDplots <- sapply(
    X = names(dataToPlot), 
    FUN = function(nameNorm) { 
      dfPlot <- dataToPlot[[nameNorm]]
      ggplot2::ggplot(
...
plotItem5 <- function(
  ...
  # Common parameters for plotting
  minY <- floor(min(sapply(dataToPlot, function(df) min(df, na.rm = TRUE))))
  maxY <- ceiling(max(sapply(dataToPlot, function(df) max(df, na.rm = TRUE))))
  colors <- normScorePalette(length(dataToPlot))
  names(colors) <- names(dataToPlot)

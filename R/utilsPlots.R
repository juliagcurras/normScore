...
plotItem1 <- function(normalizedDataList, groupData){
  ...
  dfPCV <- do.call(rbind, lapply(normalizedDataList, getPCV, groups = groups,
  ...
  if (length(groups) < 5) {
    ...
    ggplot2::ggplot(dfForest, ggplot2::aes(
      ...
      ggplot2::geom_boxplot(width = 0.65, outlier.shape = NA, na.rm = TRUE,
...
plotItem2 <- function(normalizedDataList, groupData){
  ...
  ggplot2::ggplot(dfPlot, ggplot2::aes(
    ...
    ggplot2::geom_boxplot(width = 0.65, na.rm = TRUE, outlier.shape = NA,
    ...
    ggplot2::labs(x = "Normalization Methods", y = "Intragroup correlation",
...
plotItem3 <- function(normalizedDataList, groupData, refGroup, altGroup){
  ...
  dataToPlot <- lapply(
    ...
    plotData = TRUE) #, simplify = FALSE, USE.NAMES = TRUE)
  
  # Common parameters for plotting
  minY <- floor(min(vapply(dataToPlot, function(df) 
    min(df$M, na.rm = TRUE), numeric(1))))*1.1
  maxY <- ceiling(max(vapply(X = dataToPlot, FUN = function(df) 
    max(df$M, na.rm = TRUE), FUN.VALUE = numeric(1))))*1.1
    ...
    FUN = function(nameNorm) { 
      ...
      ggplot2::ggplot(dfPlot, ggplot2::aes(
        ...
        ggplot2::labs(
          ...
          y = paste0("M: log of mean ", altGroup, " over mean ", refGroup),
          title = nameNorm) +
        ggplot2::theme_minimal(base_size = 14) + 
        ggplot2::theme(
          axis.line = ggplot2::element_line(linewidth = 0.5, colour = "black"),
...
plotItem4 <- function(normalizedDataList){
  ...
  # Common parameters for plotting
  minY <- floor(min(vapply(dataToPlot, function(df) 
    min(df$SD, na.rm = TRUE), numeric(1))))*1.1
  maxY <- ceiling(max(vapply(X = dataToPlot, FUN = function(df) 
    max(df$SD, na.rm = TRUE), FUN.VALUE = numeric(1))))*1.1
...
plotItem5 <- function(normalizedDataList){
  ...
  # Common parameters for plotting
  minY <- floor(min(vapply(dataToPlot, function(df) min(df, na.rm = TRUE), 
                           numeric(1))))
  maxY <- ceiling(max(vapply(dataToPlot, function(df) max(df, na.rm = TRUE), 
                             numeric(1))))
  colors <- normScorePalette(length(dataToPlot))
  names(colors) <- names(dataToPlot)
  ...
  allRLEplots <- lapply(
    ...
    FUN = function(nameNorm) { 
      ...
      ggplot2::ggplot(dfPlot, ggplot2::aes(
        ...
        ggplot2::geom_boxplot(width = 0.65, alpha = 0.7, 
...
plotItem6 <- function(normalizedDataList){
  ...
  # Common parameters for plotting
  minY <- floor(min(vapply(dataToPlot, 
                           function(df) min(df, na.rm = TRUE), numeric(1))))
  maxY <- ceiling(max(vapply(dataToPlot, 
    ...
    FUN = function(nameNorm) { 
      ...
      ggplot2::ggplot(dfPlot, ggplot2::aes(
        ...
        ggplot2::geom_boxplot(
          ...
          fill = colors[[nameNorm]], colour = colors[[nameNorm]]) +
        ggplot2::labs(x = "Samples", y = "Log Intensity", title = nameNorm) +
        ggplot2::ylim(c(minY, maxY)) +
        ggplot2::theme_minimal(base_size = 14) +
        ggplot2::theme(
          axis.line = ggplot2::element_line(linewidth = 0.5, colour = "black"),

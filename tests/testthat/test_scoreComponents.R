...
test_that("validateNormScoreInput stops when rawData has fewer than 100 proteins", {
  groupData <- data.frame(
    Samples = paste0("S", 1:6),
    Groups = rep(c("A", "B"), each = 3)
  )
...
test_that("validateNormScoreInput reorders normalized datasets to match rawData", {
  groupData <- data.frame(
    Samples = paste0("S", 1:6),
    Groups = rep(c("A", "B"), each = 3)
  )
...
test_that("computeNormScoreItems returns expected item columns for two groups", {
  groupData <- data.frame(
    Samples = paste0("S", 1:6),
    Groups = rep(c("A", "B"), each = 3)
  )
...
# rankNormScoreItems  ####
test_that("rankNormScoreItems computes totals, applies item0 to Log, and ranks rows", {

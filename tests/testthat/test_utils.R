...
test_that("isSameNormalization returns FALSE for numerical differences
    above tolerance", {
    x <- matrix(
        c(1, 2, 3, 4),
        nrow = 2,
...
test_that("tiMAPE returns a positive value when sample quantiles differ
    across samples", {
    dataExample <- data.frame(
        S1 = c(10, 20, 30, 40),
        S2 = c(12, 22, 32, 42),
...
test_that("tiMAPE matches the manually computed expected value", {
    ...
    sampleQuantiles <- apply(dataExample, 2, stats::quantile,
        na.rm = TRUE,
        simplify = FALSE
    )
...
test_that("meanSDdiffArea returns 0 when sample standard deviations
        are identical", {
    dataExample <- data.frame(
        S1 = c(10, 20, 30, 40),
        S2 = c(11, 21, 31, 41),
...
test_that("maDiffArea is close to zero when groups have the same values", {
    ...
    data <- cbind(
        groupValues,
        groupValues + rnorm(120 * 3, mean = 0, sd = 0.00001)
    )
...
test_that("maDiffArea returns a larger value when groups differ
        with intensity", {
    set.seed(123)
    ...
    dataSame <- cbind(
        baseData,
        baseData + rnorm(120 * 3, mean = 0, sd = 0.00001)
    )
...
test_that("getPCV returns the mean CV for each group", {
    ...
    expected <- c(
        A = mean(groupProteinCV("A", groupDataExample, dataExample),
            na.rm = TRUE
        ),
        B = mean(groupProteinCV("B", groupDataExample, dataExample),
            na.rm = TRUE
        )
    )
    ...
    expect_equal(
        getPCV(
            data = dataExample, groups = groupsExample,
...
test_that("groupProteinCV computes protein-wise CV within the selected group", {
    ...
    expect_equal(
        groupProteinCV(
            group = "A", groupData = groupDataExample,
            data = dataExample
        ),
        expected
    )
...
test_that("groupProteinCV uses only samples from the requested group", {
    ...
    expect_equal(
        groupProteinCV(
            group = "B", groupData = groupDataExample,
            data = dataExample
        ),
        c(0, 0, 0)
    )
...
test_that("withinGroupCorrelations output change for different methods", {
    ...
    resultS <- withinGroupCorrelations(dataExample, groupData,
        method = "spearman"
    )
    resultP <- withinGroupCorrelations(dataExample, groupData,
        method = "pearson"
    )
...
# computeCorrelation ####
test_that("computeCorrelation returns one named numeric value
        per normalization", {
    groupData <- data.frame(
